defmodule AOC2021.Day04.Solver do
  ...
  def solve(stream, :first), do: solve(stream, &Enum.min(&1, fn ({score1, _}, {score2, _}) -> score1 < score2 end))
  def solve(stream, :second), do: solve(stream, &Enum.max(&1, fn ({score1, _}, {score2, _}) -> score1 > score2 end))
  ...
  * map the score tho the number and multiply with all numbers of the winner board with higher draw than the board's score.
  ...
  def solve(stream, find_winner_board_by_score) do
    ...
    {winner_board_score, winner_board_index} = boards
    |> Enum.map(fn board ->
      [
        # cols
    ...
    factor = winner |> Enum.filter(&(&1 > winner_board_score)) |> Enum.map(&(draw_to_number[&1])) |> Enum.sum()
